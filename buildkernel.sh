...
sudo make ARCH=${PLATAFORM} CROSS_COMPILE=${CROSS_COMPILE}- ${BOARD_DEFCONFIG}
echo "********** Starting Linux kernel Compilation **********"

#Build linux kernel $ make ARCH=arm CROSS_COMPILE=arm-linux- zImage -j4 
sudo make ARCH=${PLATAFORM} CROSS_COMPILE=${CROSS_COMPILE}- zImage -j4
...
sudo make ARCH=${PLATAFORM} CROSS_COMPILE=${CROSS_COMPILE}- bcm2837-rpi-3-b.dtb
